...
def load_mlb_data():
    ...
    try:
        ...
        mlb_data_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'data', 'MLB_Data')
        ...
        for player_id, player_data in player_groups:
            ...
            for i in range(len(player_data)):
                ...
                # Basic batting stats
                hist_batting_avg = historical_data['batting_hits'].sum() / max(1, historical_data['batting_atBats'].sum())
                hist_obp = historical_data['batting_baseOnBalls'].sum() / max(1, historical_data['batting_plateAppearances'].sum())
                
                # Walks-specific metrics
                hist_walks = historical_data['batting_walks'].sum() / max(1, len(historical_data))
                hist_walk_rate = historical_data['batting_baseOnBalls'].sum() / max(1, historical_data['batting_plateAppearances'].sum())
                hist_walks_last_10 = historical_data.iloc[-10:]['batting_walks'].sum() / min(10, len(historical_data))
                
                # Strikeouts-specific metrics
                hist_strikeouts = historical_data['batting_strikeouts'].sum() / max(1, len(historical_data))
                hist_k_rate = historical_data['batting_strikeOuts'].sum() / max(1, historical_data['batting_plateAppearances'].sum())
                hist_ks_last_10 = historical_data.iloc[-10:]['batting_strikeouts'].sum() / min(10, len(historical_data))
                ...
                pitcher_data = pitcher_data[pitcher_data['game_date'] < current_game['game_date'].values[0]]
                ...
                if len(pitcher_data) > 0:
                    ...
                    pitcher_hist_k_per_9 = 9 * pitcher_data['pitching_strikeOuts'].sum() / max(1, pitcher_data['pitching_inningsPitched'].sum())
                    pitcher_hist_k_rate = pitcher_data['pitching_strikeOuts'].sum() / max(1, pitcher_data['pitching_battersFaced'].sum())
                    pitcher_recent_k_per_9 = 9 * pitcher_data.iloc[-3:]['pitching_strikeOuts'].sum() / max(1, pitcher_data.iloc[-3:]['pitching_inningsPitched'].sum())
                    
                    # Walk-specific pitcher metrics
                    pitcher_hist_bb_per_9 = 9 * pitcher_data['pitching_baseOnBalls'].sum() / max(1, pitcher_data['pitching_inningsPitched'].sum())
                    pitcher_hist_bb_rate = pitcher_data['pitching_baseOnBalls'].sum() / max(1, pitcher_data['pitching_battersFaced'].sum())
                    pitcher_recent_bb_per_9 = 9 * pitcher_data.iloc[-3:]['pitching_baseOnBalls'].sum() / max(1, pitcher_data.iloc[-3:]['pitching_inningsPitched'].sum())
                    
                    # Control metrics
                    pitcher_hist_k_bb_ratio = pitcher_data['pitching_strikeOuts'].sum() / max(1, pitcher_data['pitching_baseOnBalls'].sum())
                else:
...
def enhanced_preprocess_data(df, target_col, feature_engineering=True, apply_pca=False, n_components=0.95):
    ...
    if apply_pca and hasattr(pipeline, 'named_steps') and 'pca' in pipeline.named_steps:
        ...
        print(f"Explained variance ratio per component: {explained_variance[:5]}...")
...
X_walks_pca, y_walks_pca, X_walks_train_pca, X_walks_test_pca, y_walks_train_pca, y_walks_test_pca, walks_pca_pipeline = \
    enhanced_preprocess_data(walks_data, 'walks', apply_pca=True, n_components=0.95)

# Save the PCA pipeline with a different name
model_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'models')
os.makedirs(model_dir, exist_ok=True)
joblib.dump(walks_pca_pipeline, os.path.join(model_dir, 'walks_pca_pipeline.joblib'))

X_ks_pca, y_ks_pca, X_ks_train_pca, X_ks_test_pca, y_ks_train_pca, y_ks_test_pca, ks_pca_pipeline = \
    enhanced_preprocess_data(strikeouts_data, 'strikeouts', apply_pca=True, n_components=0.95)

# Save the PCA pipeline with a different name
joblib.dump(ks_pca_pipeline, os.path.join(model_dir, 'strikeouts_pca_pipeline.joblib')) 